...
class RefereeSerialNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.ui_frame_sub = self.create_subscription(
            UInt8MultiArray,
            '/referee/ui/tx_frame',
            self._ui_tx_frame_callback,
            50,
        )
        ...
        self.ui_robot_performance_pub = self.create_publisher(
            RobotPerformance, '/referee/ui/robot_performance', 10)
        self.ui_allowed_shoot_pub = self.create_publisher(
            AllowedShoot, '/referee/ui/allowed_shoot', 10)
    ...
    def _ui_tx_frame_callback(self, msg: UInt8MultiArray) -> None:
        """发送 UI 节点转发过来的原始串口帧"""
        if not self.serial_port or not self.serial_port.is_open:
            return
        if not msg.data:
        ...
        try:
            ...
            if written != len(frame):
                self.get_logger().warn(
                    f'UI帧写入长度不完整: {written}/{len(frame)}'
                )
        except serial.SerialException as e:
    ...
    def _update_constraint_state(self, cmd_id: int, data: Any) -> None:
        ...
        if cmd_id == CommandID.ROBOT_PERFORMANCE:
            ...
            ui_msg = self._create_ros_message(cmd_id, data)
            if ui_msg:
                self.ui_robot_performance_pub.publish(ui_msg)
            ...
            ui_msg = self._create_ros_message(cmd_id, data)
            if ui_msg:
                self.ui_allowed_shoot_pub.publish(ui_msg)
            updated = True
